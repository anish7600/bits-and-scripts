#!/usr/bin/env python3
"""
Generate concise, interview-ready cheat sheets for a list of Linux commands
in both Markdown and HTML formats using OpenAI's GPT-4o via the Responses API.
One .md and one .html file per command.

The output is beautified for readability, and HTML output has no extra ```html fences.

Usage:
  python generate_linux_cheatsheets.py \
      --input commands.txt \
      --outdir cheatsheets \
      --model gpt-4o \
      --max-workers 4
...
PROMPT_TEMPLATE = (
    """Explain the Linux command `{command}` in a clear, concise way. Skip the full man page format and focus on the most useful options. Provide:
...
MARKDOWN_INSTRUCTIONS = (
    "You are a concise Linux expert. Output valid, well-formatted Markdown only, using proper headings, tables, and fenced code blocks."
)

HTML_INSTRUCTIONS = (
    "You are a concise Linux expert. Output valid, indented HTML only, with semantic tags (h1, h2, table, code, pre). Do not include extra explanations or markdown code fences."
)
...
def generate_one(client: OpenAI, model: str, cmd: str, outdir: Path, retries: int = 2) -> List[Path]:
    prompt = build_prompt(cmd)
    attempt = 0
    last_exc: Exception | None = None
    while attempt <= retries:
        try:
            md = call_gpt(client, model, prompt, MARKDOWN_INSTRUCTIONS)
            html = call_gpt(client, model, prompt, HTML_INSTRUCTIONS)
...
def main(argv: List[str] | None = None) -> int:
    ...
    print(f"Generating {len(commands)} Markdown & HTML cheat sheets to {args.outdir.resolve()} using {args.model}...")

    with concurrent.futures.ThreadPoolExecutor(max_workers=args.max_workers) as pool:
        futures = [
            pool.submit(generate_one, client, args.model, cmd, args.outdir, args.retries)
            for cmd in commands
        ]
